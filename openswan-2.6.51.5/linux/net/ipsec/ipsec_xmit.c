...
ipsec_xmit_encap_init(struct ipsec_xmit_state *ixs)
{
	 ...
	 */
	if (new_version == osw_ip_hdr_version(ixs))/*如果ip族相同使用原先头部信息(例如ipid)*/
...
ipsec_xmit_esp_ah(struct ipsec_xmit_state *ixs)
{
	...
	if (ixs->ixt_a) {
		ipsec_alg_sa_esp_hash(ixs->ipsp,
				(caddr_t)ixs->espp, ixs->len - ixs->iphlen - ixs->authlen,/*认证除iphdr, authlen以外的报文*/
...
ipsec_xmit_init2(struct ipsec_xmit_state *ixs)
{
	...
	while (ixs->ipsp) {
		...
		case IPPROTO_ESP:
			...
			if ((ixs->ixt_e=ixs->ipsp->ips_alg_enc)) {/*加密算法需要的头部空间和块大小*/
				ixs->blocksize = ixs->ixt_e->ixt_common.ixt_blocksize;
				ixs->headroom += ESP_HEADER_LEN + ixs->ixt_e->ixt_common.ixt_support.ias_ivlen/8;
			} else{
...
#ifdef NAT_TRAVERSAL
/*
*	如果期间经过了NAT( ixs->natt_type >0 ),则进行NAT-T封装:
*根据在ipsec_xmit_init2()中计算的长度进行调整，将IP头部之后的数据报文进行后移，然后添加UDP封装信息
*/
enum ipsec_xmit_value ipsec_nat_encap(struct ipsec_xmit_state *ixs)
{
	if (ixs->natt_type && ixs->natt_head) {
		...
		if(skb_tailroom(ixs->skb) < ixs->natt_head) {/*将报文内容后移，因此需要tailroom空间足够装下NAT-T封装长度*/
		...
		skb_put(ixs->skb, ixs->natt_head);/*skb_put:添加尾部*/
...
ipsec_xmit_send(struct ipsec_xmit_state *ixs)
{
	...
	__skb_pull(ixs->skb, skb_network_header(ixs->skb) - ixs->skb->data);/*去二层头*/
	if(!ixs->pass) {
