...
ipsec_tunnel_strip_hard_header(struct ipsec_xmit_state *ixs)
{
	...
	/* head----data---tail---end*/
	if(ixs->hard_header_len < 0) {/*这是个错误的报文，应该被丢弃: iph >= data*/
	...
	if(ixs->hard_header_len == 0) { /* no hard header present */
		...
		ixs->hard_header_len = ixs->physdev->hard_header_len;/* 0 */
	}
...
ipsec_tunnel_SAlookup(struct ipsec_xmit_state *ixs)
{
...
#endif /* CONFIG_KLIPS_IPV6 */
	{
		...
		if ((ntohs(osw_ip4_hdr(ixs)->frag_off) & IP_OFFSET) == 0)
			nexthdroff = (ixs->iph + (osw_ip4_hdr(ixs)->ihl<<2)) - (void *)ixs->skb->data;
		...
		if (debug_tunnel & DB_TN_XMIT) {/*此处为了调试，将网络IP地址转换为点分十进制的形式*/
	...
	/*
	 * ixs->matcher用来匹配eroute表，在上述处理过程中已经获取到了报文的源目的IP、协议族、载荷类型等
	 */
	 ...
	 */
	spin_lock_bh(&eroute_lock);/*eroute操作上锁*/
	
	/*根据报文信息ixs->matcher来查找eroute路由表*/
	ixs->eroute = ipsec_findroute(&ixs->matcher);

	/*再次从报文中提取端口，并将其保存在ixs->sport, ixs->dport中*/
	...
	{/*IPV4处理流程*/
		 ...
		 */
		 /*
		 * 如果为本地外发的、协议为500/4500端口的UDP报文，则应该为IKE协商报文，应该将协议修改为SPI_PASS
		 */
	...
	if (bypass==FALSE && ixs->eroute) {/*此过程处理非IKE报文*/
		ixs->eroute->er_count++;
...
ipsec_tunnel_start_xmit(struct sk_buff *skb, struct net_device *dev)
{
	...
	/*网络数据包(skb)的正确性检查:1.是否支持V6, 2.是否需要支持IP选项字段*/
