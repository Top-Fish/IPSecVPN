...
/*kernel_alg_esp_enc_ok主要检查两个方面:
 *	1. 支持该算法
 *	2. 秘钥长度合乎要求
 *	这个函数需要注意的是第三个参数alg_info并未使用。而alg_info是我们配置的策略，
 *	所以说这里判断是否支持某种算法，指的是系统是否支持，而不只是说此策略是否支持。
 *  也就是说:在openswan源码中配置的策略支持在发起端才会使用，在响应端是不会严格使用的，
 *	即使策略上没有配置也可能是协商成功。
 */
err_t
                          ...
                          struct alg_info_esp *alg_info __attribute__((unused)))
{
       ...
       */
          int ret=ESP_EALG_PRESENT(alg_id);/*是否支持此算法*/
          if (!ret) goto out;/*不支持该算法*/
        ...
        if(alg_id == ESP_AES_GCM_8
           ...
           || alg_id == ESP_AES_GCM_16) {
            if(key_len != 128 && key_len!=192 && key_len!=256 ) {/*AES-GCM算法的长度要求必须为128, 192, 256(单位bit)*/
