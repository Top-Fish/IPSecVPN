       ...
       , u_int8_t np)
{
    ...
    if(oakley_mode) {
		...
		/*根据配置的秘钥算法信息重新生成一个sadb信息*//*传入的sadb应该为固定的秘钥算法信息，因此需要根据策略来重新生成一个新的sadb*/
    ...
    struct state *st)          /* current state object */
{
    ...
    if (selection && proposal.isap_notrans != 1)/*应答报文中的SA只能包含一个建议载荷*/
    {
	...
	/*变换载荷的属性部分，*/
          ...
          while (pbs_left(&trans_pbs) != 0)
          {
              ...
              passert((a.isaat_af_type & ISAKMP_ATTR_RTYPE_MASK) < 32);/*现有的属性类型*/

              if (LHAS(seen_attrs, a.isaat_af_type & ISAKMP_ATTR_RTYPE_MASK))/*记录属性载荷是否重复*/
              {
              ...
              switch (a.isaat_af_type)
              {
                    case OAKLEY_ENCRYPTION_ALGORITHM | ISAKMP_ATTR_AF_TV:
                        ...
                        /* if (ike_alg_enc_present(val)) { */
                              ta.encrypt = val;
                              ta.encrypter = crypto_get_encrypter(val);/*如果此加密算法支持，则获取加密算法信息，并存储*/
                              ta.enckeylen = ta.encrypter->keydeflen;
                        } else
#endif
                        switch (val)/*采用默认的加密算法?  以下为必须支持的算法*/
                        {
#if defined(USE_1DES)
                        case OAKLEY_DES_CBC:
...
#ifdef IKE_ALG
                        if (ike_alg_hash_present(val)) {/*是否支持此hash算法*/
                              ta.prf_hash = val;
...
/* #endif */
                    ...
                    case OAKLEY_AUTHENTICATION_METHOD | ISAKMP_ATTR_AF_TV:
                      ...
                      lset_t iap = st->st_policy & POLICY_ID_AUTH_MASK;
                        ...
                        case OAKLEY_PRESHARED_KEY:/*预共享秘钥*/
#ifdef XAUTH
                        psk:
                              ...
                              if ((iap & POLICY_PSK) == LEMPTY)/*策略是否支持预共享秘钥*/
                              {
                              ...
                              else
                              {
                                  ...
                                  if (get_preshared_secret(con) == NULL)/*预共享秘钥是否存在*/
                                  {
                                        char mid[IDTOA_BUF]
                                            , hid[IDTOA_BUF];
    ...
    struct state *st)                    /* current state object */
{
    ...
    /* DOI */
    if (sa->isasa_doi != ISAKMP_DOI_IPSEC)/*DOI固定为ISAKMP_DOI_IPSEC*/
    {
    ...
    /* Situation 也是固定值，为SIT_IDENTITY_ONLY*/
    if (!in_struct(&ipsecdoisit, &ipsec_sit_desc, sa_pbs, NULL))
          return SITUATION_NOT_SUPPORTED;
...
/*解析建议载荷*/
    ...
    while (next_full)/*一个SA中可能包含多个建议载荷，再此进行遍历*/
    {
	  ...
	  */
          ...
          do {
              ...
              if (next_proposal.isap_protoid == PROTO_IPCOMP)/*IPComp协议*/
              {
                    /* IPCOMP CPI */
             ...
             * IPCOMP???
             */
                    if (ntohl(next_spi) < IPSEC_DOI_SPI_MIN)/*SPI范围应该>255*/
                    {
              ...
              if (next_proposal.isap_notrans == 0)/*属性载荷个数*/
              {
		...
		*建议载荷协议类型
		*
		*建议载荷的头部已经解析为next_proposal
		*建议载荷的内容部分尚未解析，起始位置为next_proposal_pbs
		*/
              switch (next_proposal.isap_protoid)/*使用哪种封装协议在此进行协商*/
              {
              case PROTO_IPSEC_AH:
                    if (ah_seen)
                    {
		...
		/*在建议载荷中np只有两种类型:空、建议载荷;不会存在其他类型
		*如果后续还有其他类型的载荷，应该在建议载荷的上一层:SA头部中的np
		*/
              /* refill next_proposal */
              if (next_proposal.isap_np == ISAKMP_NEXT_NONE)/*没有后续的建议载荷,将next_full清零*/
              {
                    next_full = FALSE;
                    break;
              }
              ...
              if (!in_struct(&next_proposal, &isakmp_proposal_desc, sa_pbs, &next_proposal_pbs))
                    return BAD_PROPOSAL_SYNTAX;
          } while (next_proposal.isap_proposal == propno);/*这个可以包含多个建议载荷???,
      sa_pbs已经指向下一个SA载荷， next_proposal_pbs指向未解析的变换载荷部分。*/
       ...
       */
          ...
          if (ah_seen)
          {
              ...
              for (tn = 0; tn != ah_proposal.isap_notrans; tn++)/*解析变换载荷,一般包含多个变换载荷*/
              {
                    int ok_transid = 0;
                    bool ok_auth = FALSE;
             ...
             */
                    switch (ah_attrs.transattrs.integ_hash)/*AH协议选用的认证算法*/
                    {
              ...
              if (ah_attrs.encapsulation == ENCAPSULATION_MODE_TUNNEL)/*隧道模式? or not*/
                    tunnel_mode = TRUE;
          }
          ...
          if (esp_seen)
          {
              ...
              for (tn = 0; tn != esp_proposal.isap_notrans; tn++)/*遍历ESP算法参数*/
              {
...
#ifdef KERNEL_ALG
                    if(c->alg_info_esp) {/*如果连接上配置了ESP算法，则需要判断是否支持此算法*/
...
#warning "Building with ESP-Null"
                              case ESP_NULL:
                                  if (esp_attrs.transattrs.integ_hash == AUTH_ALGORITHM_NONE)/*加密算法和认证算法不能同时为空*/
                                    {
                              ...
                              case ESP_DES:          /* NOT safe */
                                  loglog(RC_LOG_SERIOUS, "1DES was proposed, it is insecure");/*支持DES算法，但是它不安全*/
                              default:
			...
			/*hash算法*/
                    ...
                    if(ugh != NULL) {/*不支持此认证算法，记录日志信息*/
                        switch (esp_attrs.transattrs.integ_hash)
                              {
                              case AUTH_ALGORITHM_NONE:/*ESP要么支持认证要么和AH结合使用AH的认证算法*/
                                  if (!ah_seen)
                                    {
                                        DBG(DBG_CONTROL | DBG_CRYPT
                    ...
                    if (ah_seen && ah_attrs.encapsulation != esp_attrs.encapsulation)/*如果AH与ESP的认证方式不同，则退出*/
                    {
       ...
       */
              ...
              if (esp_attrs.encapsulation == ENCAPSULATION_MODE_TUNNEL)/*封装模式:隧道模式or传输模式*/
                    tunnel_mode = TRUE;
          }/*以下为异常信息处理:*/
          else if (st->st_policy & POLICY_ENCRYPT)/*需要加密但是没有ESP载荷*/
          {
              DBG(DBG_CONTROL | DBG_CRYPT
          ...
          else if ((st->st_policy & POLICY_AUTHENTICATE) && !ah_seen)/*需要认证但是没有AH载荷*/
          {
              DBG(DBG_CONTROL | DBG_CRYPT
		...
		/*
		*如果为AH+ESP的隧道封装方式，则应该有两个proposal载荷???
		*/
              /* AH proposal */
              if (ah_seen)
