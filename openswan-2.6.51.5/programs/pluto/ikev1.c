...
void process_packet_tail(struct msg_digest **mdp)
{
	...
	if (st->st_skey_ei.ptr == (u_char *) NULL)/*尚未协商加密材料，无法解密*/
	{
	 ...
	 */
	{
	    const struct encrypt_desc *e = st->st_oakley.encrypter;/*使用加密密钥进行解密*/

	    if (pbs_left(&md->message_pbs) % e->enc_blocksize != 0)/*填充后的加密块大小有误*/
	    {
	    ...
	    {
		if(st->st_iv_len == 0) {
		    init_phase2_iv(st, &md->hdr.isa_msgid);/*msgid 和 st_ph1_iv*/
		} else {
		    /* use old IV */
     ...
     */
    {
	struct payload_digest *pd = md->digest;/*用来存放载荷信息*/
	...
	{
	    struct_desc *sd = payload_desc(np);/*获取载荷的描述信息*/
	...
	if (needed != 0)/*如果不为0，则说明必须要求的载荷没有填充，错误的报文*/
	{
	...
	&& md->hdr.isa_np != ISAKMP_NEXT_SA)/*第一阶段如果存在SA载荷，则必须在ISAKMP头部之后*/
	{
	 ...
	 */
/*
*	第二阶段: hash载荷紧跟isakmp载荷，hash载荷之后为IPSec SA载荷
*/
...

	{
	    ...
	    i = 1;
	    while(p != NULL) {/*md->digest[i]中存放的是报文中的载荷信息；依次顺序存放*/
		if (p != &md->digest[i])
		{
	 ...
	 */
	{
	    ...
	    if (id != NULL)
	    {
		if (id->next == NULL || id->next->next != NULL)/*id载荷为成对出现的*/
		{
	...
	while(p != NULL) {/*获取VID,主要为NATT相关，这里选用VID值大的nat标准*/
    ...
    /* possibly fill in hdr *//*填充ISAKMP头部。这里只需要回转收到的报文头部即可。条件是下一个载荷不为空*/
