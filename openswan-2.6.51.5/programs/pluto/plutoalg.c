...
alg_info_snprint_esp(char *buf, size_t buflen, struct alg_info_esp *alg_info)
{
	...
	ALG_INFO_ESP_FOREACH(alg_info, esp_info, cnt) {
	    if (kernel_alg_esp_enc_ok(esp_info->esp_ealg_id, 0, NULL)) {/*支持该加密算法?, 秘钥长度是否合法?*/
		DBG_log("esp algid=%d not available", esp_info->esp_ealg_id);
		continue;
	    }

	    if (kernel_alg_esp_auth_ok(esp_info->esp_aalg_id, NULL)) {/*支持该认证算法?, 秘钥长度是否合法?*/
		DBG_log("auth algid=%d not available", esp_info->esp_aalg_id);
		continue;
	    }

	    eklen=esp_info->esp_ealg_keylen;/*加密长度，如果未设置，则使用默认的加密长度*/
	    if (!eklen)
		eklen=kernel_alg_esp_enc_keylen(esp_info->esp_ealg_id)*BITS_PER_BYTE;
		
	    aklen=esp_info->esp_aalg_keylen;/*认证长度，如果未设置，则使用默认的认证长度*/
	    if (!aklen)
...
alg_info_snprint_ah(char *buf, size_t buflen, struct alg_info_esp *alg_info)
{
	...
	ALG_INFO_ESP_FOREACH(alg_info, esp_info, cnt) {/*遍历算法信息*/

	    if (kernel_alg_esp_auth_ok(esp_info->esp_aalg_id, NULL)) {/*是否支持此认证算法*/
		DBG_log("auth algid=%d not available", esp_info->esp_aalg_id);
		continue;
	    }

	    aklen=esp_info->esp_aalg_keylen;/*是否需要使用默认的认证长度*/
	    if (!aklen)
		aklen=kernel_alg_esp_auth_keylen(esp_info->esp_aalg_id)*BITS_PER_BYTE;
...
alg_info_snprint_phase2(char *buf, size_t buflen, struct alg_info_esp *alg_info)
{
	...
	alg_info_snprint_esp(buf, buflen, alg_info);/*转换ESP算法*/
	return;
    case PROTO_IPSEC_AH:
	alg_info_snprint_ah(buf, buflen, alg_info);/*转换AH算法*/
	return;
    default:
...
kernel_alg_policy_algorithms(struct esp_info *esp_info)
{
    ...
    switch(ealg_i) {
        ...
        case ESP_CAST:
            break;
        default:
            if (!esp_info->esp_ealg_keylen) {/*如果未填充加密秘钥长度，则填充为最大的秘钥长度*/
                /**
		  ...
		  , bool logit)
{
	...
	if(policy & POLICY_ENCRYPT) {
	    ...
	    if (!ESP_EALG_PRESENT(ealg_i)) {/*如果该加密算法不支持，则直接错误退出*/
		if(logit) {
	...
	aalg_i=alg_info_esp_aa2sadb(esp_info->esp_aalg_id);/*认证算法*/
	if (!ESP_AALG_PRESENT(aalg_i)) {/*如果该加密算法不支持，则直接错误退出*/
	...
	/* 	do algo policy *//*检查秘钥长度*/
	kernel_alg_policy_algorithms(esp_info);

	if(policy & POLICY_ENCRYPT) {/*只能加密?*/
	    /*	open new transformation */
	    db_trans_add(db_ctx, ealg_i);/*填充策略中配置的加密算法信息*/
	...
	} else if(policy & POLICY_AUTHENTICATE) {/*只能认证?*/
	    /*	open new transformation */
...
kernel_alg_db_new(struct alg_info_esp *alg_info, lset_t policy, bool logit)
{
	...
	/*	pass aprox. number of transforms and attributes *//*分配建议载荷空间*/
	...
	/* passert(alg_info!=0); */
	if (alg_info) {/*策略上配置的算法信息*/
		ALG_INFO_ESP_FOREACH(alg_info, esp_info, i) {/*遍历配置的算法信息，将其添加到ctx_new*/
		    bool thistime;
		    tmp_esp_info = *esp_info;
		    thistime = kernel_alg_db_add(ctx_new
						 , &tmp_esp_info
						 , policy, logit);
		    if(thistime == FALSE) {/*不支持该算法*/
			success=FALSE;
		    }
		}
	} else {/*默认算法*/
		ESP_EALG_FOR_EACH_UPDOWN(ealg_i) {
			tmp_esp_info.esp_ealg_id=ealg_i;/*加密算法*/
			tmp_esp_info.esp_ealg_keylen=0;
			ESP_AALG_FOR_EACH(aalg_i) {
				tmp_esp_info.esp_aalg_id=alg_info_esp_sadb2aa(aalg_i);/*认证算法*/
...
kernel_alg_makedb(lset_t policy, struct alg_info_esp *ei, bool logit)
{
    ...
    if(ei == NULL) {/*如果策略中没有配置第二阶段的算法参数，则使用默认值*/
		struct db_sa *sadb;
...
/*
*	第二阶段配置了算法策略，注意这里的ei已经不可能为NULL了
*/
    dbnew=kernel_alg_db_new(ei, policy, logit);/*根据策略，填充第二阶段的加密算法等信息，并存储在dbnew中*/
    ...
    /* make a fresh copy */
    n = sa_copy_sa(&t, 0);/*拷贝所有的sa载荷信息*/
    n->parentSA = FALSE;

    db_destroy(dbnew);/*sa载荷已经拷贝，因此可以将此变量空间释放*/
