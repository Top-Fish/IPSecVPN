	     ...
	     , u_int8_t np)
{
    ...
    pbs_set_np(outs, ISAKMP_NEXT_v2SA);/*设置上一个载荷的np参数*/
    ...
    )
{
    ...
    while(np == ISAKMP_NEXT_P) {/*一般情况下只有一个建议载荷，但是实际使用过程中可能包含很多个*/
	/*
	...
	{
	    /* as it should be *//*正常情况下建议载荷中的spisize为0*/
	}
...
/*DH组编号*/
    ...
    st->st_oakley = ta;/*算法信息存储在st_oakley上*/
