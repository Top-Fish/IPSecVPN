...
/*¼ÇÂ¼±¾µØËíµÀ½Ó¿ÚµÄIPºÍ¶Ë¿Ú*/
     ...
     */
    if(md->chain[ISAKMP_NEXT_v2N]) {/*½âÎö±¨ÎÄÖÐµÄÍ¨ÖªÔØºÉ*/
        ikev2_process_notifies(st, md);

        /* switch to port 4500, if necessary */
        ikev2_update_nat_ports(st);/*Èç¹û´æÔÚNAT,Ôò½øÐÐ¶Ë¿Ú¸¡¶¯*/
    }


    /* check if the responder replied with v2N with DOS COOKIE */
    /*
	* Èç¹ûÏìÓ¦¶Ë¶Ô·¢Æð¶Ë½øÐÐcookie challenge¿¹¹¥»÷¼ì²â£¬Ô
	* ÔòÓ¦´ð±¨ÎÄÖÐ»á°üº¬ÀàÐÍÎªCOOKIEµÄÍ¨ÖªÔØºÉ
	* ÕâÊ±Ó¦¸Ã½«dos cookie´æ´¢µ½×´Ì¬ÉÏºóÖØÐÂ·¢ÆðÐ­ÉÌ
	*
	*---ChallengeÔØºÉÓ¦¸ÃÓëNATTÔØºÉ²»»áÍ¬Ê±³öÏÖ£¬·ñÔò»ñÈ¡µ½µÄv2NÔØºÉÓÐÎÊÌâ
	*/
    ...
    /* process and confirm the SA selected */
    {
        ...
        /* SA body in and out *//*½âÎö¶Ô¶ËÑ¡ÖÐµÄSAÔØºÉ*/
    ...
    /* update state *//*¸üÐÂ×´Ì¬ºÍmsgidÐòºÅ*/
                            ...
                            , struct pluto_crypto_req *r)
{
    ...
    finish_dh_v2(st, r);/*·Ç³£ÖØÒªµÄº¯Êý£¬½«Éú³ÉµÄÃÜÔ¿´æ´¢ÔÚstateÉÏ*/
    ...
    pst = st;
    ret = allocate_msgid_from_parent(pst, &mid);/*·ÖÅämsgid£¬ ÀàËÆÓÚTCPÖÐµÄÐòºÅ*/
    if(ret != STF_OK) {
        /*
    ...
    authstart = reply_stream.cur;/*´ÓÍ·¿ªÊ¼ÈÏÖ¤*/
    ...
    e.isag_np = ISAKMP_NEXT_v2IDi;/*IKEv2Í¨ÓÃÔØºÉÍ·*/
    ...
    if(!out_struct(&e, &ikev2_e_desc, &md->rbody, &e_pbs)) {/*ºóÐøÔØºÉÍ¨¹ýe_pbsÀ´¼ÌÐøÌî³ä*/
        return STF_INTERNAL_ERROR;
    }
    ...
    /* insert IV *//**/
    {
	    ...
	    get_rnd_bytes(iv, ivsize);/*Ëæ»úÉú³É³õÊ¼»¯ÏòÁ¿IV£¬Ìî³äµ½±¨ÎÄÖÐ*/
    }
    /* note where cleartext starts */
    init_sub_pbs(&e_pbs, &e_pbs_cipher, "cleartext");/*¼ÓÃÜ²¿·ÖÊ¹ÓÃe_pbs_cipherÀ´Ìî³ä*/
	...
	/*¹¹ÔìIDÔØºÉ*/
        ...
        pbs_set_np(&e_pbs_cipher, ISAKMP_NEXT_v2IDi);/*ÉèÖÃÉÏÒ»¸öÔØºÉµÄNP×Ö¶Î*/
        id_start = e_pbs_cipher.cur;
        if (!out_struct(&r_id /*Ìî³äIDÔØºÉÍ·²¿ÐÅÏ¢: idÀàÐÍ£¬Èçipv4,fqdn,...*/
                        , &ikev2_id_desc
                        , &e_pbs_cipher
                        , &r_id_pbs)) {
            return STF_INTERNAL_ERROR;
...

	{
	        ...
	        hmac_final(idhash, &id_ctx);/*ÏÂÃæµÄÈÏÖ¤ÔØºÉÓÃµ½¸ÃÖµ*/
	}
	...
	*	4. ¶Ô¶ËÇëÇó±¾¶Ë·¢ËÍÖ¤Êé
	*/
    if(doi_send_ikev2_cert_thinking(st)) {/*ÊÇ·ñÐèÒª·¢ËÍÖ¤ÊéÀàÔØºÉ: CERT, CERTREQ*/
    ...
    /* send out the AUTH payload *//*±¨ÎÄÖÐÓÐÒ»¸öµ¥¶ÀµÄÈÏÖ¤ÔØºÉ*/
    {
        lset_t policy;
	...
	/*Ìî³äÈÏÖ¤ÔØºÉÊý¾Ý(²»Í¬ÓÚ±¨ÎÄ×îºóµÄÍêÕûÐÔÔØºÉ)
	*
	...
	**/
		...
		/*½«±¾¶ËºÍ¶Ô¶Ë±£»¤×ÓÍø×ª»»Îªts½á¹¹£¬Ìî³äµ½stÉÏ*/
	    ...
	    ikev2_calc_emit_ts(md, &e_pbs_cipher, INITIATOR, next_payload, c0, policy);
            ...
            if( !(st->st_connection->policy & POLICY_TUNNEL) ) {/*´«ÊäÄ£Ê½*/
...
stf_status ikev2parent_inR1(struct msg_digest *md)
{
    ...
    /* check if the responder replied with v2N with DOS COOKIE */
    if( md->chain[ISAKMP_NEXT_v2N] ) {/*Ê×ÏÈ½âÎö±¨ÎÄÖÐµÄÍ¨ÖªÔØºÉ*/
	...
	/*±éÀúËùÓÐµÄ½¨ÒéÔØºÉ*/
        for(notify=md->chain[ISAKMP_NEXT_v2N]; notify!=NULL; notify=notify->next) {
            ...
            case v2N_NO_PROPOSAL_CHOSEN:/*½¨ÒéÔØºÉ²»ºÏÊÊ*/
                action="SA deleted";
                break;
            case v2N_INVALID_KE_PAYLOAD:/*DH×é²Â´íÇé¿öÏÂ»á·µ»Ø´ËÌáÊ¾ÐÅÏ¢*/
                action="SA deleted";
                break;
            default:
                break;
