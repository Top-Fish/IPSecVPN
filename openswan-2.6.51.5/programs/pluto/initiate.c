...
{
    c->newest_isakmp_sa = serial;/*建立ISAKMP SA就是简单的处理下???*/

    if (uniqueIDs/*如果ID标识必须独一无二，那么需要检查当前所有的连接信息*/
#ifdef XAUTH
	...
	)
{
	...
	for (d = connections; d != NULL; )
	{
	    ...
	    && same_id(&c->spd.this.id, &d->spd.this.id)/*标识相同*/
	    && same_id(&c->spd.that.id, &d->spd.that.id)
	    && (!sameaddr(&c->spd.that.host_addr, &d->spd.that.host_addr)/*NAT之后可能有多台隧道端点，因此如果IP相同再检查port是否相同*/
		|| (c->spd.that.host_port != d->spd.that.host_port))/*IP相同、端口不同*/
