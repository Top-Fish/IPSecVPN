		...
		, unsigned int np, pb_stream *outpbs)
{
    ...
    send_certreq = 0;/*不发送证书请求载荷???*/

    if(send_certreq){/*需要发送证书请求载荷*/
        cert.isac_critical = ISAKMP_PAYLOAD_NONCRITICAL;
...
doi_send_ikev2_cert_thinking(struct state *st)
{
    ...
    /* decide to send_cert or not *//*确定是否需要发送证书*/
    send_cert = (c->policy & POLICY_RSASIG)/*采用证书认证方式*/
	&& mycert.type != CERT_NONE/*并且使用的证书无误*/
