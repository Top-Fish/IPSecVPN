...
/*
*用来填充NAT-D载荷
* v2N_NAT_DETECTION_DESTINATION_IP   : 对端
* v2N_NAT_DETECTION_SOURCE_IP			: 本端
*/
bool justship_v2nat(struct state *st, pb_stream *outpbs)
{
    ...
    setchunk(hash_chunk, digest, SHA1_DIGEST_SIZE);/*20字节*/
...
{
    if(st->hidden_variables.st_nat_traversal & NAT_T_DETECTED) {/*路径中存在NAT设备，则进行端口浮动*/
...
/*
*	根据Ni、发起端IP、发起端SPI、以及时间随机值生成的字符串(COOKIE)
*  时间种子在此为固定值!!!!
*/
static bool ikev2_get_dcookie(u_char *dcookie,  chunk_t st_ni
                              ,ip_address *addr, u_int8_t *spiI)
