...
decode_peer_id(struct msg_digest *md, bool initiator, bool aggrmode)
{
    ...
    const pb_stream *const id_pbs = &id_pld->pbs;/*id的数据部分*/
    struct isakmp_id *const id = &id_pld->payload.id;/*id的头部信息*/
    struct id peer;
 ...
 * +-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
 */
    if (!(id->isaid_doi_specific_a == 0 && id->isaid_doi_specific_b == 0)/*如果两个有一个不为0，需要检查协议和端口*/
    && !(id->isaid_doi_specific_a == IPPROTO_UDP && id->isaid_doi_specific_b == IKE_UDP_PORT))/*如果没有NAT-T,那么必须使用UDP/500端口进行通讯*/
    {
...
/*提取对端标识信息*/
     ...
     */
    st->st_peeridentity_protocol = id->isaid_doi_specific_a;/*协议ID: TCP--6   UDP-17*/
    st->st_peeridentity_port = ntohs(id->isaid_doi_specific_b);/*端口*/

    {
	char buf[IDTOA_BUF];

	idtoa(&peer, buf, sizeof(buf));/*将id信息转换为字符串*/
    ...
    if (initiator)
    {
		if (!same_id(&st->st_connection->spd.that.id, &peer))/*比较连接上的对端标识和实际对端标识，不同的话协商错误，直接退出*/
		{
    ...
    else
    {
		...
		/* check for certificate requests *//*解析签名证书*/
	...
	DBG(DBG_CONTROL,
	    ...
	    DBG_log("offered CA: '%s'", buf));
	        ...
	        if (r->kind == CK_TEMPLATE || r->kind == CK_GROUP) {/*如果为模板，则实例化一个连接*/
