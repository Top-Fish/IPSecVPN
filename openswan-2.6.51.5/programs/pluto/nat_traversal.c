...
void nat_traversal_natoa_lookup(struct msg_digest *md, struct hidden_variables *hv)
{
	...
	else if (!(hv->st_nat_traversal & LELEM(NAT_TRAVERSAL_NAT_BHND_PEER))) {/*对端如果没有在NAT之后，则不需要OA*/
		loglog(RC_LOG_SERIOUS, "NAT-Traversal: received %d NAT-OA. "
			"ignored because peer is not NATed", i);
		return;
	}
	else if (i>1) {/*如果收到多个OA,则只是用第一个OA----origial address*/
		loglog(RC_LOG_SERIOUS, "NAT-Traversal: received %d NAT-OA. "
			"using first, ignoring others", i);
	}

	/** Take first **/
	p = md->chain[ISAKMP_NEXT_NATOA_RFC];/*重新定位到OA的位置，其实就是第一个OA*/
			     ...
			     struct state *st, bool initiator)
{
	...
	if (!out_modify_previous_np(nat_np, outs)) {/*修改前一个载荷的np载荷*/
		return FALSE;
	}
	...
	{
		pb_stream pbs;
		if (!out_struct(&natoa, &isakmp_nat_oa, outs, &pbs))/*填充OA头部描述信息*/
			return FALSE;

		if (!out_raw(ip_val, ip_len, &pbs, "NAT-OAi"))/*填充OA数据*/
			return FALSE;
...
void nat_traversal_change_port_lookup(struct msg_digest *md, struct state *st)
{
	 ...
	 */
	if ( ((st->st_state == STATE_MAIN_I3)/*从第5个报文开始，如果需要，则都需进行端口浮动*/
	     || (st->st_state == STATE_QUICK_I1)
	     || (st->st_state == STATE_AGGR_I2))
	    && (st->hidden_variables.st_nat_traversal & NAT_T_WITH_PORT_FLOATING)
	    && (st->hidden_variables.st_nat_traversal & NAT_T_DETECTED)
	    && (st->st_localport != NAT_T_IKE_FLOAT_PORT))/*需要进行端口滑动，但尚未进行端口滑动*/
	{
	    DBG(DBG_NATT
	 ...
	 * 如果当前网口的IP和端口与当前需要的不一致，则重新查找合适的网口
	 */
...
/*遍历所有的interface, 找到IP和端口都能满足的接口，并使用之*/
	    for (i = interfaces; i !=  NULL; i = i->next) {
		if ((sameaddr(&st->st_localaddr, &i->ip_addr))&& (st->st_localport == i->port))
		{
		    DBG(DBG_NATT,
