...
generate_msgid(struct state *isakmp_sa)
{
    ...
    for (;;)
    {
	get_rnd_bytes((void *) &msgid, sizeof(msgid));/*随机生成msgid*/
	if (msgid != 0 && unique_msgid(isakmp_sa, msgid))/*生成的msgid应该保持唯一，否则重新生成。100次后不再尝试*/
	    break;
...
insert_state(struct state *st)
{
    ...
    *p = st;
     ...
     */
    if (st->st_event == NULL)
	event_schedule(EVENT_SO_DISCARD, 0, st);/*为什么要丢弃事件呢???*/
