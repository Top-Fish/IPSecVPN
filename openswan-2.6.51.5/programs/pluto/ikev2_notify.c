                               ...
                               , chunk_t *data)
{
    ...
    if(same_chunk(*data, calculated_hash)) {/*本端计算hash值与对端计算hash值一致，说明未经过NAT*/
        ...
        st->hidden_variables.st_nat_traversal = NAT_T_WITH_RFC_VALUES |
            NAT_T_DETECTED;/*默认使用的最新的RFC标准的NAT-T*/
...
stf_status ikev2_process_notifies(struct state *st, struct msg_digest *md)
{
    ...
    for(p = md->chain[ISAKMP_NEXT_v2N]; p != NULL; p = p->next) {/*每次解析一个通知载荷*/
		
      payload_name = enum_name(&ikev2_notify_names, p->payload.v2n.isan_type);
	  
      if(payload_name == NULL) {
        if(p->payload.v2n.isan_type < v2N_INITIAL_CONTACT) {/*IKEv1的类型*/
      ...
      switch(p->payload.v2n.isan_type) {/*检查是否经过NAT-T，报文中包含两个NATT通知载荷*/
      case v2N_NAT_DETECTION_DESTINATION_IP:
      case v2N_NAT_DETECTION_SOURCE_IP:
              ...
              u_int16_t type, chunk_t *n_data, pb_stream *rbody)
{
    ...
    pbs_set_np(rbody, ISAKMP_NEXT_v2N);/*在这里填充的HDR中的np字段，且类型为ft_np_in*/
    ...
    n.isan_spisize = 0;
    if(spi) {
        n.isan_spisize = spi->len;/*长度有问题???,spi->len可能为0*/
    }
    n.isan_type = type;
