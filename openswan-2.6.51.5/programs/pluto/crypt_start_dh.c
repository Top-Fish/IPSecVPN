			     ...
			     , u_int16_t oakley_group2)
{
    ...
    pluto_crypto_copychunk(&dhq->thespace, dhq->space, &dhq->secret, st->st_sec_chunk);
...
/*
*此函数与第一阶段的start_dh_secretiv()函数并没有太大差异
*使用DH算法涉及到与共享密钥、Nonce载荷、KE载荷、COOKIE等重要参数。
*/
 ...
 * invoke helper to do DH work.
 *
 *	Ni、Nr, gi, gr, icookie, rcookie, st_sec_chunk
 *
 */
		       ...
		       , u_int16_t oakley_group2)
{
    ...
    pcr_init(&r, pcr_compute_dh_v2, importance);/*初始话*/
    ...
    /* convert appropriate data to dhq */
    dhq->auth 		= st->st_oakley.auth;        /* auth ? integ_hash */
    dhq->prf_hash   	= st->st_oakley.prf_hash;
